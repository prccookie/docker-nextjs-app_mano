...
export function SortableList({ user }: TodolistProps) {
    ...
    const handleReverse = () => {
        ...
        setmessageIndex((prev) => (prev + 1) % messages.length); // 任意で進める
        runMessageCycle(); // ← 明示的に再起動
    }
        ...
        const appear = setInterval(() => {
            ...
            if (i >= nextMessage.length) {
                ...
                disappearTimer.current = setTimeout(() => {
                    ...
                    disappearInterval.current = disappear;
                }, 10000);
                setmessageIndex((prev) => (prev + 1) % messages.length);
            }
        }, 70);
        appearInterval.current = appear;
    };
